...
def generate_game_pages(json_folder, output_folder):
    ...
    game_template = env.get_template("games_normal.html")
    games = []
...
def generate_home_page(output_folder, games):
    home_template = env.get_template("games_home.html")
    output_html = home_template.render(games=games)
    with open(os.path.join(output_folder, "index.html"), "w", encoding="utf-8") as htmlfile:
        htmlfile.write(output_html)


# Define paths
json_folder = "../data/games/normal"
